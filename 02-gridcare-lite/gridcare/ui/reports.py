...
class ReportsTab(ttk.Frame):
    ...
    def load(self):
        ...
        for i, (label, value) in enumerate(cells):
            ...
            ttk.Label(box, text=str(value), font=("", 14, "bold")).pack(padx=12, pady=6)

        severity = summary["open_by_severity"]
        if severity:
            order = [s for s in services.SEVERITIES if s in severity]
            text = "   ".join(f"{s}: {severity[s]}" for s in order)
        else:
            text = "none"
        ttk.Label(self.stats, text=f"Unresolved by severity:  {text}").grid(
            row=1, column=0, columnspan=6, sticky="w", padx=6, pady=(2, 6))
        ...
        if regions:
            ...
            self.ax.barh(names, counts, color="#2b6cb0")
            self.ax.invert_yaxis()
            self.ax.set_xlabel("Unresolved outages")
            self.ax.set_title("Unresolved outages by region")
            # Whole outages only - a gridline at 2.5 outages just looks broken.
            self.ax.xaxis.get_major_locator().set_params(integer=True)
        else:
            self.ax.text(0.5, 0.5, "No unresolved outages", ha="center", va="center")
